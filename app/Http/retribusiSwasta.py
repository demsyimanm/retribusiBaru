...
try:
    ...
    while True:
        try:
            sql_retribusi   = '''select id, pelanggan_id, tgl_lunas, bulan, tahun from retribusiswasta where status_cek = 0'''
            tanda           = 0
            ...
            if (tanda==1):
                ...
                if (unchecked is not None):
                    ...
                    sql             = '''select * from tunggakanswasta where pelanggan_id ='''+str(no_plg_ret)+'''having min(pelanggan_id)'''
                    ...
                    if (ret_plg is not None):
                        ...
                        insert              = '''insert into lunasswasta (pelanggan_id, nama, jalan, gang, nomor, notamb, da, kd_tarif, retribusi, listrik, lbr_jalan, periode_tagih, ketstatus, tgl_lunas, bulan, tahun) values('''+tunggak[1]+''','''+str(tunggak[2])+''','''+str(tunggak[3])+''','''+str(tunggak[4])+''','''+str(tunggak[5])+''','''+str(tunggak[6])+''','''+str(tunggak[7])+''','''+str(tunggak[8])+''','''+tunggak[9]+''','''+tunggak[10]+''','''+tunggak[11]+''','''+tunggak[12]+''','''+str(tunggak[13])+''','''+str(tgl_lunas)+''','''+str(bulan)+''','''+str(tahun)+''')'''
                        insert_exec         = cursor_tunggak.execute(insert)
                        delete              = '''delete from tunggakanswasta where id='''+str(tunggak[0]) 
