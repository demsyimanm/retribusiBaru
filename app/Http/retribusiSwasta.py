...
def check(bot, top, index):
    ...
    while (bot <= top):
        if (bot>top):
            sys.exit(0)
        try:
            sql_retribusi         = '''select id, pelanggan_id, tgl_lunas, bulan, tahun from retribusiswasta where status_cek = 0 and id between '''+str(bot)+''' and '''+str(top)
            tanda                 = 0
            ...
            if (tanda==1):
                ...
                if (unchecked is not None):
                    ...
                    sql             = '''select * from tunggakanswasta where pelanggan_id ='''+str(no_plg_ret)+''' having min(id)'''
                    ...
                    if (tunggak is not None):
                        ...
                        insert              = '''insert into lunasswasta (pelanggan_id, nama, jalan, gang, nomor, notamb, da, kd_tarif, retribusi, listrik, lbr_jalan, periode_tagih, ketstatus, tgl_lunas, bulan, tahun) values('''+str(pelanggan_id)+''',"'''+str(nama)+'''","'''+str(jalan)+'''","'''+str(gang)+'''","'''+str(nomor)+'''","'''+str(notamb)+'''","'''+str(da)+'''","'''+str(kd_tarif)+'''",'''+str(retribusi)+''','''+str(listrik)+''','''+str(lbr_jalan)+''','''+str(periode_tagih)+''',"'''+str(ketstatus)+'''","'''+str(tgl_lunas)+'''","'''+str(bulan)+'''","'''+str(tahun)+'''")'''
                        insert_exec         = cursor_retribusi.execute(insert)
                        delete              = '''delete from tunggakanswasta where id='''+str(id_tunggak) 
                        delete_exec         = cursor_tunggak.execute(delete)

                        if (insert_exec and delete_exec):
                            update_status           = '''update retribusiswasta set status_cek = 1 where id = '''+str(id_retribusi)
...
try:
    ...
    db_retribusi          = MySQLdb.connect("10.151.63.12", "retribusi", "retribusi", "retribusi")
    cursor_retribusi      = db_retribusi.cursor()
    sqlSum                = '''select count(*) from retribusiswasta where status_cek=0'''
    sqlSum_exec           = cursor_retribusi.execute(sqlSum)
    sqlSumData            = cursor_retribusi.fetchone()
    minID                 = '''select min(id) from retribusiswasta where status_cek=0'''
